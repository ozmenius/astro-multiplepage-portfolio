...
1. Claude Code generates n8n workflows. Describe the automation in plain language. Claude Code outputs valid n8n workflow JSON that you import directly into the canvas. Best for complex multi-step workflows where the logic is non-trivial. This emerging pattern sidesteps manual canvas-clicking entirely.

2. n8n calls the Claude API. Add an Anthropic Chat Model node to any n8n workflow. Claude handles the reasoning or generation step. n8n handles scheduling, credentials, error handling, and downstream actions. Best for embedding AI reasoning inside a reliable automated process.

3. Claude Code writes the logic, n8n runs it. Use Claude Code to write complex transformation or scoring logic as a script, then call that script from an n8n Code node or via HTTP. This separates smart code from reliable orchestration across the lifecycle.
